	...
	public static void bindColumns(final Element node, final SimpleValue simpleValue,
		...
		if ( autoColumn && simpleValue.getColumnSpan() == 0 ) {
			...
			if(simpleValue instanceof ManyToOne){
				columnName= mappings.getNamingStrategy().propertyToColumnName( propertyPath+"Id" ) ;
			    logicalName=columnName;// mappings.getNamingStrategy().logicalColumnName( null, propertyPath +"_id");
			}else{
				columnName= mappings.getNamingStrategy().propertyToColumnName( propertyPath ) ;
				logicalName= mappings.getNamingStrategy().logicalColumnName( null, propertyPath );
			}
