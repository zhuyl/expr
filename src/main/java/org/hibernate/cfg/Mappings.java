...
//			final List extendsQueue,
	...
	public void addClass(PersistentClass persistentClass) throws MappingException {
		PersistentClass existed=(PersistentClass)classes.get( persistentClass.getEntityName());
		if(null!=existed){
			try {
				if(Class.forName(existed.getClassName()).isAssignableFrom(Class.forName(persistentClass.getClassName()))){
					log.info(existed.getClassName()+" replaced by "+persistentClass.getClassName());
