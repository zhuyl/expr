...
<#assign labInfo><@text name="保险资产信息" /></#assign>
  <table width="90%" align="center" class="formTable">
    ...
    <tr>
      ...
      <td width="80%">
       <select  name="insurancePolicyAnswer.policy.insurant"  style="width:250px;"  >
      <#list members as member>
      <option value="${member.name}" <#if member.name=(insurancePolicyAnswer.policy.insurant)!>selected</#if>>
     ...
     <td><input type="text" name="insurancePolicyAnswer.policy.applyOn" maxlength="10" value="${(insurancePolicyAnswer.policy.applyOn?string('yyyy-MM-dd'))?if_exists}" onfocus="calendar()"/></td>
      ...
      <td width="80%">
         ...
         <#list InsuranceTypes as insuranceType>
             <option value="${insuranceType.id}" <#if ((insurancePolicyAnswer.policy.type.parent.id)!(0))=insuranceType.id>selected</#if> >${insuranceType.name}</option>
         </#list>
         </select>
         <#--insuranceProduct.type.id-->
         <select name="insurancePolicyAnswer.policy.type.id" id="insuranceProductType" style="width:100px"></select>
      </td>
    </tr>  
    <tr>
 ...
 name="insurancePolicyAnswer.policy.time.id" style="width:50%;"><option value="">请选择</option></@>
  ...
  <tr>
      ...
      <td width="80%"><@htm.radio2  name="insurancePolicyAnswer.policy.additional" value=(insurancePolicyAnswer.policy.additional)!(false)/></td>
 </tr>
    <tr>
...
<script language="javascript" > 
    ...
    function save(form){
      var a_fields = {
         ...
         'insurancePolicyAnswer.policy.company':{'l':'保险公司', 'r':true, 't':'f_company'},
         'insurancePolicyAnswer.policy.insurant':{'l':'被保险人物', 'r':true, 't':'f_insurant'},
         'insurancePolicyAnswer.policy.applyOn':{'l':'投保日期', 'r':true, 't':'f_applyOn'},
         'insurancePolicyAnswer.policy.type.id':{'l':'保险类型', 'r':true, 't':'f_type'},         
         'insurancePolicyAnswer.policy.coverage':{'l':'保额', 'r':true, 't':'f_coverage','f':'money'},
         'insurancePolicyAnswer.policy.expense':{'l':'年保费', 'r':true, 't':'f_expense','f':'money'},
         'insurancePolicyAnswer.policy.payTime.id':{'l':'缴费年限', 'r':true, 't':'f_payTime'},   
         'insurancePolicyAnswer.policy.payType.id':{'l':'缴费方式', 'r':true, 't':'f_payType'},   
         'insurancePolicyAnswer.policy.time.id':{'l':'保险期限', 'r':true, 't':'f_time'}
               
        
      };
	  var v = new validator(form , a_fields, null);
	  if (v.exec()) {
	  	if(form['insurancePolicyAnswer.policy.additional'][0].checked && form['insurancePolicyAnswer.master.id'].value==''){
	  		alert("该附加险应当填写主险");
	  		return;
	  	}
	    form.submit();
   ...
   var defaultInsuranceTypeId="${(insurancePolicyAnswer.policy.type.id)?default(0)}";
   ...
   <#if (insurancePolicyAnswer.policy.type.parent)??>
      getSubTypes(${insurancePolicyAnswer.policy.type.parent.id});
   </#if>
</script>
 </body> 
</html>