...
<#assign labInfo><@text name="理财目标" /></#assign>
  <table width="90%" align="center" class="formTable">
    ...
    <input name="aimItemResult.id" type="hidden" value="${aimItemResult.id!}"/>
    ...
    <tr>
      ...
      <td width="80%">
 <@htm.i18nSelect datas=aimTypes selected="${(aimItemResult.aimtype.id)?if_exists}" 
   name="aimItemResult.aimtype.id" style="width:50%;"><option value="">请选择</option></@></td>
    </tr>  
     <tr>
      <td width="20%" id="f_content" class="title"><font color="red">*</font>目标内容:</td>
      <td width="80%">
      <textarea rows="5" cols="70" id="content" name="aimItemResult.content">${(aimItemResult.content)?if_exists}</textarea></td>
    </tr>  
...
<script language="javascript" > 
    function save(form){
         var a_fields = {
             'aimItemResult.aimtype.id':{'l':'目标类型', 'r':true, 't':'f_aimtype'},
             'aimItemResult.content':{'l':'目标内容', 'r':true, 't':'f_content'}
         };
