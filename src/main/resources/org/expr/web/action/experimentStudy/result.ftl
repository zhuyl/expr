...
<#include "../experimentCheck/metadata.ftl"/>
...
<h3 align="center">${result.student.name}(${result.student.code})的${result.experiment.lesson.coursename}--${result.experiment.name}作业<#if experiment.publishScore!false><br>总分:${result.score!}</#if></h3>
...
<table width="90%" align="center"><tr><td>
	...
	<#assign typeUri>result/${typeUriMap[analysis.code]}!infolet.action?type=${typeMap[analysis.code]!}&std.id=${result.student.id}&experiment.id=${Parameters['experiment.id']}${extParams[analysis.code]!}&nopage=1</#assign>
	<@sj.div cssClass="ui-widget-content ui-corner-all" id="div${analysis.id}"  href="${typeUri}">
	</@>
	<div align="right"><#if accessments[analysis.code]??><#assign item=accessments[analysis.code]>优(90~100):${item.excellent}; 良(80~89):${item.good}; 中(70~79):${item.middle}; 及格(60~69):${item.pass}; 不及格(0～59):${item.nopass}; </#if> <input name="typeName" value="${typeMap[analysis.code]}" type="hidden"/>
	<#if experiment.publishScore!false>分数:<span id="${typeMap[analysis.code]}Score"></span><input type="hidden" value="${mark.mark!}" id="${typeMap[analysis.code]}Mark"></#if>
	</div>
	</#if>
	</#list>
	</@>
