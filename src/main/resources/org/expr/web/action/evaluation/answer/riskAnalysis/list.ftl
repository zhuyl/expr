...
<table id="toolbar"></table>
  <table class="formTable" align="center" width="80%">
	<form name="editForm" method="post" action="${base}/answer/riskAnalysis!saveRemark.action">
...
<@table.table id="memberTable" width="80%" align="center">
 <@table.thead>
   <@table.selectAllTd id="riskItemAnswerId" />
   <@table.td text="产品类型"/>
   <@table.td text="风险分析"/>
 </@>
 <@table.tbody datas=answers;answer>
   <@table.selectTd id="riskItemAnswerId" value=answer.id type="checkbox"/>
   <td>${(answer.financetype.name)!}</td>
   <td>${(answer.content)!}</td>
   </@>
 </@>

<@htm.actionForm name="actionForm" action="riskAnalysis.action" entity="riskItemAnswer">
  <input type="hidden" name="caze.id" value="${Parameters['caze.id']}"/>
</@>
