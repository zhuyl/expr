<#include "/template/head.ftl"/>
 ...
 <script>  
  function incomeExpense(year){
     self.location="analysisChartsStat.action?method=incomeExpense&year="+year+"&caze.id=${Parameters['caze.id']}";
  }
 var select ="<select name='year' onchange='incomeExpense(this.value)'><#list 1..planYears as i> <option value='${i}' <#if i?string=analysisAnswer.year!1> selected</#if>>${i}</option></#list ></select>"
