  ...
  function changePassword(){
   ...
   <div id="user" class="module expanded">
     ...
     <div class="modulebody">
     	欢迎您：<#if Session['user.key']?exists>${Session['user.key'].fullname?if_exists}</#if>     
     	今天是${date}	<br>
     	<br>
     </div>
   </div>
   ...
   <div id="downloadFileList" class="module expanded">
     ...
     <table  style="font-size:10pt" width="90%">
      ...
      <#list downloadFileList as file>
	   <tr>
	    <td><image src="${base}/static/images/file/${extMap[file.fileExt]?default("generic.gif")}">&nbsp;<A style="color:blue" href="download.action?method=download&document.id=${file.id}">${file.name}</A></td>
