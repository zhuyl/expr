...
<table id="toolbar"></table>
	...
	<form name="actionForm" method="post" action="${base}/answer/consumePlan!save.action">
		<input type="hidden" name="params" value="&caze.id=${Parameters['caze.id']}">
		<input type="hidden" name="caze.id" value="${Parameters['caze.id']}">
		<tr><td class="title" id="f_consume"><font color="red">*</font>日常消费支出</td><td><input id="consume" name="consume"/></td></tr>
  ...
  <script language="javascript">
     var bar = new ToolBar("toolbar","日常消费支出规划",null,true,true);
  ...
  <script language="javascript" > 
    function save(form){
         var a_fields = {
             'consume':{'l':'日常消费支出','r':true,'t':'f_consume','f':'unsignedReal'},
