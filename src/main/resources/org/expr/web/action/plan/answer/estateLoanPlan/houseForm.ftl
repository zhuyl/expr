...
<table id="toolbar"></table>
	...
	<form name="actionForm" method="post" action="${base}/answer/estateLoanPlan!saveHouseForm.action">
		<input type="hidden" name="params" value="&caze.id=${Parameters['caze.id']}">
		<input type="hidden" name="caze.id" value="${Parameters['caze.id']}">
		<tr><td class="title" id="f_expense"><font color="red">*</font>房产价值</td><td><input id="expense" name="expense" value="${(answer.houseCapital)!}"/></td></tr>
  ...
  <script language="javascript">
     var bar = new ToolBar("toolbar","房产价值评估",null,true,true);
    ...
    function save(form){
         var a_fields = {
             'expense':{'l':'房产价值','r':true,'t':'f_expense','f':'unsignedReal'},
