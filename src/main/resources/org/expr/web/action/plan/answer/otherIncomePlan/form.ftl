...
<table id="toolbar"></table>
	...
	<form name="actionForm" method="post" action="${base}/answer/otherIncomePlan!save.action">
		<input type="hidden" name="params" value="&caze.id=${Parameters['caze.id']}">
		<input type="hidden" name="caze.id" value="${Parameters['caze.id']}">
		<tr><td class="title" id="f_income"><font color="red">*</font>其他收入</td><td><input id="income" name="income"/></td></tr>
  ...
  <script language="javascript" > 
    function save(form){
         var a_fields = {
             'income':{'l':'其他收入','r':true,'t':'f_income','f':'unsignedReal'},
